...
# ============================================================
def fit_bootstrap_iteration(i, traces, time_grid):
    """Single bootstrap iteration - to be run in parallel"""
    try:
        sampled = np.random.choice(list(traces.keys()), size=len(traces), replace=True)
        boot_df = pd.DataFrame({aid: traces[aid] for aid in sampled})
        boot_mean = boot_df.reindex(time_grid).mean(axis=1)

        t = boot_mean.index[boot_mean.index >= 0].values
        y = boot_mean.loc[boot_mean.index >= 0].values
    ...
    def run(self):
        ...
        try:
            ...
            for gname, g in df.groupby(group_cols):
                ...
                # Bootstrap fitting
                self.progress.emit(f"  Running {self.params['boot']} bootstrap iterations...")
            
                fit_func = partial(fit_bootstrap_iteration, traces=traces, time_grid=time_grid)
                
                with Pool(processes=self.params['n_cores']) as pool:
                    results = pool.map(fit_func, range(self.params["boot"]))
